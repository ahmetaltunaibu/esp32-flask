...
# app.py'e bu debug endpoint'ini ekle:
...
# Firmware check endpoint'ini debug mesajlarıyla güncelle:
...
def check_firmware(cihaz_id):
    ...
    try:
        with get_db() as conn:
            ...
            print(f"🔍 DEBUG - Firmware Check")
            print(f"Cihaz ID: {cihaz_id}")
            print(f"Device Found: {device is not None}")

            if device:
                print(f"Current Version: {device['current_version']}")
                print(f"Target Firmware: {device['target_firmware']}")
                print(f"File Path: {device['file_path']}")
                print(f"Firmware Active: {device['firmware_is_active']}")
            else:
            ...
            print(f"Karşılaştırma: Current={current_version}, Target={target_version}")

            # Güncelleme gerekiyor mu?
            if target_version and target_version != current_version and device['file_path']:
                base_url = request.url_root.rstrip('/')

                print(f"✅ Güncelleme mevcut: {current_version} -> {target_version}")
                ...
                return jsonify({
                    ...
                    "debug": f"Update: {current_version} -> {target_version}"
                })

            print(f"❌ Güncelleme yok")
        ...
        print(f"❌ Firmware Check Hatası: {str(e)}")
        return jsonify({
            "error": str(e),
            "update_available": False,
