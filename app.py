...
def index():
    ...
    try:
        with get_db() as conn:
            ...
            # Admin veya normal kullanıcı kontrolü
            ...
            for cihaz in cihazlar_raw:
                ...
                # ✅ NONE KONUM DEĞERLERİNİ DÜZELTİLMİŞ KONTROL
                if cihaz_dict.get('konum') is None or cihaz_dict.get('konum') == '':
                    cihaz_dict['konum'] = 'Belirtilmemiş'
                
                # Fabrika adı kontrolü
                if cihaz_dict.get('fabrika_adi') is None or cihaz_dict.get('fabrika_adi') == '':
                    cihaz_dict['fabrika_adi'] = 'Belirtilmemiş'
                
                # Cihaz adı kontrolü
                if cihaz_dict.get('cihaz_adi') is None or cihaz_dict.get('cihaz_adi') == '':
                    cihaz_dict['cihaz_adi'] = cihaz_dict.get('cihaz_id', 'Bilinmeyen Cihaz')

                # Her sensörün en son değerini al
                sensor_data = conn.execute('''
        ...
        # Hata durumunda boş liste ile template'i render et
        return render_template('index.html', cihazlar=[], error_message="Cihaz listesi yüklenirken hata oluştu")
